...
-- TABLE: system_health
-- ============================================================================
-- Purpose: Keep-Alive mechanism to prevent Supabase project pausing
-- Strategy: GitHub Action pings /api/system/pulse every 48 hours, which
--           updates this table to maintain "activity" status
-- ============================================================================
...
INSERT INTO public.system_health (status)
VALUES ('healthy')
ON CONFLICT DO NOTHING;

-- Index for fast lookup (only one row expected)
CREATE INDEX IF NOT EXISTS idx_system_health_last_check 
  ON public.system_health(last_check DESC);

COMMENT ON TABLE public.system_health IS 
  'Stores system health pings. Updated by /api/system/pulse to prevent DB pausing.';
...
-- Policy: Public can read system health (for status page)
CREATE POLICY "Allow public read on system_health"
  ON public.system_health
  FOR SELECT
  TO anon, authenticated
  USING (true);

-- Policy: Only service role can update (for /api/system/pulse)
CREATE POLICY "Allow service role update on system_health"
  ON public.system_health
  FOR UPDATE
  TO service_role
  USING (true)
